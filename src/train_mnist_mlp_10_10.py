...
2) 训练/评估双层 MLP
3) 按验证准确率保存最佳模型到 models/mnist_mlp_10_10

Usage examples:
    python src/train_mnist_mlp_16_16.py
    python src/train_mnist_mlp_16_16.py --epochs 20 --batch-size 128
    python src/train_mnist_mlp_16_16.py --device cuda
"""
...
def parse_args() -> argparse.Namespace:
    """解析命令行参数，方便快速调参。"""
    parser = argparse.ArgumentParser(description="Train a 2-hidden-layer MLP (10,10) on MNIST")
    ...
    # 模型输出目录。
    parser.add_argument("--output-dir", type=str, default="models/mnist_mlp_10_10")
...
class MLP10x10(nn.Module):
    """MNIST 双隐藏层 MLP（10, 10）。"""
...
def main() -> None:
    ...
    model = MLP10x10().to(device)
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    best_acc = 0.0
    best_ckpt_path = output_dir / "best_mnist_mlp_10_10.pt"
