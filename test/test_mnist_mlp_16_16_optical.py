"""Evaluate 2-hidden-layer MLP (10,10) with optical matmul on MNIST test split.

该脚本参照 BNN 测试脚本实现，针对训练脚本输出的 checkpoint 做推理：
1) 使用本地 MNIST 测试集推理（默认先跑 1k）
...
def parse_args() -> argparse.Namespace:
    """定义命令行参数。"""
    parser = argparse.ArgumentParser(description="Evaluate MNIST MLP (10,10) with optical matmul")
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="models/mnist_mlp_10_10/best_mnist_mlp_10_10.pt",
