"""Evaluate 2-hidden-layer MLP (80,20) with optical matmul on MNIST test split.

该脚本参照 BNN 测试脚本实现，针对训练脚本输出的 checkpoint 做推理：
1) 使用本地 MNIST 测试集推理（默认先跑 1k，现设定 default 为 10,000）
...
def parse_args() -> argparse.Namespace:
    """定义命令行参数。"""
    parser = argparse.ArgumentParser(description="Evaluate MNIST MLP (80,20) with optical matmul")
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="models/mnist_mlp_80_20/best_mnist_mlp_80_20.pt",
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.2,
        help="Dropout probability used during training (needed to reconstruct the model).",
    )
...
def main() -> None:
    ...
    model = MLP80x20(dropout=args.dropout).to("cpu")
